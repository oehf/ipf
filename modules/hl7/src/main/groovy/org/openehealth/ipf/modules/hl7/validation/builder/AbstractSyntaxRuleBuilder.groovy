...
public class AbstractSyntaxRuleBuilder extends MessageRuleBuilder{
     ...
     AbstractSyntaxRuleBuilder(String version, DefaultValidationContext context, String messageType, String triggerEvent, Object... args) {
         ...
         staticRules.msgType = messageType
         staticRules.msgStructID = Parser.getMessageStructureForEvent("${messageType}_${triggerEvent}", version)
         analyzeSegments(staticRules, 1, args)
         checkIf { 
             HL7Exception[] exceptions = new AbstractSyntaxValidator().validate(it, staticRules)
             exceptions.asType(List.class).collect { new ValidationException(it.message, it) } as ValidationException[]
         }
     }
