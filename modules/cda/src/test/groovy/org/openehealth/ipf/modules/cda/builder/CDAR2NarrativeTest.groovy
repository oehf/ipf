...
public class CDAR2NarrativeTest extends AbstractCDAR2BuilderTest {
  ...
  public void testRenderMultimediaSingleReference() {
    ...
    assert text.mixed[1].value.referencedObject[0] == 'MMS'
  }
  ...
  public void testRenderMultimediaMultiReference() {
    StrucDocText text = builder.build {
      strucDocText('Erythematous rash, palmar surface, left index finger.') {
        renderMultiMedia(referencedObject: ['MM1', 'MM2'])  // multi-valued primitive attribute
      }
    }
    assert text != null
    assert text.text == 'Erythematous rash, palmar surface, left index finger.'
    assertEquals 2, text.mixed[1].value.referencedObject.size
