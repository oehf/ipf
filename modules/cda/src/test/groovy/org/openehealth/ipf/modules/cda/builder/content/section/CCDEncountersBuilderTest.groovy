 ...
 */
public class CCDEncountersBuilderTest extends AbstractCDAR2BuilderTest {
	
	@Before
	void initialize() throws Exception {
	    new CCDDefinitionLoader(builder).loadEncounters(loaded)
        new CCDEncountersExtension(builder).register(registered)
	}
	
	@Test
	public void testCCDEncounters() {
		def POCDMT000040Section encounters = builder.build(
		        getClass().getResource('/builders/content/section/CCDEncountersExample.groovy'))
		new CCDEncountersValidator().validate(encounters, null)
	}
