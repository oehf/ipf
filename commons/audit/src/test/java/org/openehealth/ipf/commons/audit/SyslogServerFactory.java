 ...
 *
 */
public interface SyslogServerFactory {

    static Verticle createUDPServer(String host, int port, Async async) {
        return new UDPSyslogServer(host, port, async);
    }

    static Verticle createTCPServer(int port, Async async) {
        return new TCPSyslogServer(port, async);
    }

    static Verticle createTCPServerOneWayTLS(int port, String keystorePath, String keystorePassword, Async async) {
        return new TCPSyslogServer(port, "NONE", null, null, keystorePath, keystorePassword, async);
    }

    static Verticle createTCPServerTwoWayTLS(int port,
