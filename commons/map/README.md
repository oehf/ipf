...
`CustomMappingsConfigurer` collects every `CustomMappings` bean into it, which is how an
application layers its own mappings over the ones IPF ships. Two ways to layer:
...
configurer adds a resource that names one of them.
