 ...
 * {@code finishBeanFactoryInitialization} and thus before the {@code ContextRefreshedEvent} that
 * starts e.g. the Camel context, so that mappings can already be resolved while beans are being
 * initialized. Contributions can be ordered relative to each other with Spring's {@code @Order} /
 ...
 * <p>
 * Unlike {@link SpringBidiMappingService}, this holds the model's constraints: a duplicate mapping
 * name is an error unless the later declaration says {@code override}, and a value with two
 * inverses is an error rather than a reverse lookup that silently answers whichever key was
 * written last. Both can be relaxed with {@link #setAllowOverride} and
...
public class SpringMappings extends DefaultMappings
     ...
     * @throws IllegalArgumentException if the resource cannot be read and
    ...
    public synchronized void setMappingResource(Resource resource, String format) {
        ...
        try {
            ...
            log.debug("Ignoring unreadable mapping resource {}", resource.getFilename());
        }
    }

    /**
     * @return the format this bean's resources are read in, {@code null} if each is dispatched by
     * its file extension
     */
     ...
     *                      id}, or {@code null} to dispatch by file extension
     */
    ...
    /**
     * Forgets every mapping and re-registers the functions this bean was configured with.
     */
    @Override
    public synchronized void clear() {
        super.clear();
        resources.clear();
        functions.forEach(functions()::register);
    }

    /**
     * @return whether a resource that cannot be read is skipped instead of failing the context
     */
