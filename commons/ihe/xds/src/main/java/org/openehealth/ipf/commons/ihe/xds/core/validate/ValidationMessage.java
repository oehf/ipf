...
public enum ValidationMessage {
    ...
    TOO_MANY_SUBMISSION_SET_STATES("Exactly one submission set status slot must be present in SS-DE Association"),
    INVALID_SUBMISSION_SET_STATUS("Wrong submission set status in SS-DE Association"),
    ...
    NODE_REPRESENTATION_MISSING("Classification does not specify its node representation. Scheme = %1s"),
    NODE_REPRESENTATION_PROHIBITED("Classification shall not specify a node representation. Scheme = %1s"),
    ...
    TOO_MANY_VALUES_FOR_QUERY_PARAMETER("Too many values for query parameter: %1s", ErrorCode.STORED_QUERY_PARAM_NUMBER),
    ...
    SUBMISSION_SET_STATUS_MANDATORY("Submission set status slot is missing in SS-DE Association"),
