...
/**
 * Base class for the AuditEvents of transactions that create a resource and are profiled on the BALP
 * PatientCreate pattern, doing everything derivable from the audit message generically. A
 ...
 * <p>
 * Like the query patterns, the client agent carries 110153 and the server agent 110152.
 * <p>
 ...
 */
public abstract class SelfInitializingCreateAuditEvent extends PatientCreateAuditEvent implements SelfInitializing {
    ...
    /**
     * Fills in everything the BALP PatientCreate pattern requires and the audit message provides: the
     * time the event was recorded, the create subtype, the agents, the audit source, the patient, and
    ...
    public void initialize(AuditMessage auditMessage) {
        ...
        // the pattern requires a create subtype next to the transaction subtype the subclass adds
        addSubtype()
