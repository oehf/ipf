 ...
 * @since 4.8
 * <p>
 * TODO merge with IgBasedInstanceValidator
 */
public abstract class BaseValidator extends FhirTransactionValidator.Support {
    ...
    public BaseValidator(FhirContext fhirContext, String... packagePaths) {
        log.debug("Initializing Validator for {}", (Object) packagePaths);
        this.validator = createValidator(fhirContext, packagePaths);
        log.debug("Initialized Validator for {}}", (Object) packagePaths);
    }
     ...
     *                   profiles under the key {@link Constants#INTERACTION_REQUEST_VALIDATION_PROFILES}
     ...
     *                   profiles under the key {@link Constants#INTERACTION_RESPONSE_VALIDATION_PROFILES}
     * @return an {@link OperationOutcome} containing the validation results, including any issues
     *         found during validation (never {@code null})
     * @throws UnprocessableEntityException if the resource fails validation against the configured profiles
     */
     ...
     * The order of validation supports is important - they are consulted in the order added.
     *
     * @param fhirContext the FHIR context
    ...
    private ValidationSupportChain createValidationSupportChain(FhirContext fhirContext, String... packagePaths) {
        ...
        addNpmPackageSupport(fhirContext, supportChain, packagePaths);
        
        // 2. Default profile validation support for base FHIR resources
        supportChain.addValidationSupport(new DefaultProfileValidationSupport(fhirContext));
        
        // 3. Snapshot generation for differential profiles
        supportChain.addValidationSupport(new SnapshotGeneratingValidationSupport(fhirContext));
        
        // 4. Common code systems (e.g., LOINC, SNOMED CT stubs)
     ...
     * This should be added first to ensure custom profiles take precedence.
     *
     * @param fhirContext  the FHIR context
