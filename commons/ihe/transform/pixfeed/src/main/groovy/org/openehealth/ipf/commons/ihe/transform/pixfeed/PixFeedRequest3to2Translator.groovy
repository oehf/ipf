...
import org.openehealth.ipf.modules.hl7dsl.PrimitiveAdapter
import org.openehealth.ipf.modules.hl7dsl.GroupAdapter
import groovy.util.slurpersupport.GPathResultimport ca.uhn.hl7v2.model.Message

import org.openehealth.ipf.commons.ihe.transform.core.Hl73to2RequestTranslator
 ...
 */
class PixFeedRequest3to2Translator implements Hl73to2RequestTranslator {
     ...
     * <p>
	 * On 'PID-18' account number (recoginzed as such by the means of 
	 * accountNumberRoot) will be copied to PID-18 ot the output.
	 ...
	 * nationalIdentifierRoot) will be copied to PID-19 ot the output.
	 * On other values account number will not be copied. 
	 */
     ...
     */
    MessageAdapter translateRequest(String xmlText) {
        ...
        */
        ...
        if (triggerEvent == 'A40') {
            ...
            def priorIdRoot               = priorId.@root.text()
            def priorIdExtension          = priorId.@extension.text() 
            def priorIdAssigningAuthority = priorId.@assigningAuthorityName.text()
            fillCx(grp.MRG[1](0), priorIdRoot, priorIdExtension, priorIdAssigningAuthority)
        }
