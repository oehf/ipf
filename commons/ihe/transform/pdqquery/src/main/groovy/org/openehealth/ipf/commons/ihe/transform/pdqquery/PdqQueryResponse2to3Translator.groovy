...
import org.openehealth.ipf.commons.ihe.transform.core.Hl72to3ResponseTranslator
import org.openehealth.ipf.commons.ihe.transform.core.XmlYielder
import static org.openehealth.ipf.commons.ihe.transform.core.Utils.*

/**
 * PDQ Response translator HL7 v2 to v3.
 * @author Marek Václavík, Dmytro Rud
 */
class PdqQueryResponse2to3Translator implements Hl72to3ResponseTranslator {

    /**
     * Predefined fix value of
     * <code>//registrationEvent/custodian/assignedEntity/id/@root</code>.
     * In productive environemnts to be set to the <code>id/@root</code>
     ...
     * In productive environemnts to be set to the <code>id/@extension</code>
     * of the device representing the MPI.
     */
    ...
    /**
     * If true, the transaltion will handle DSC segments from the PDQv2 interface. 
     * Otherwise the presence of the DSC semgent will cause errors.
     */
     ...
     */
    String translateResponse(MessageAdapter rsp, String origMessage) {
                ...
                'xmlns:xsd':  'http://www.w3.org/2001/XMLSchema') 
        {
            ...
            acknowledgement {
                ...
                // AcknowledgementDetailType: 2.16.840.1.113883.5.1082
                // codeSystem: HL7 2.5, HL7 Table 0357
                if (status.ackCode[1] != 'A') {
                    acknowledgementDetail(typeCode: 'E') {
                        code(code: status.errorCode, codeSystem: this.errorCodeSystem)
                                    ...
                                    subject1(typeCode: 'SBJ') {
                                        patient(classCode: 'PAT') {
                                            ...
                                            patientPerson {
                                                name {
                                                    def pid5   = qr.PID[5]
                                                    def family = pid5[1][1].value
                                                    def given  = pid5[2].value
                                                    def middle = pid5[3].value
