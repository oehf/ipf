 ...
 */
class PdqQueryRequest3to2Translator implements Hl73to2RequestTranslator {
    ...
    public MessageAdapter translateRequest(String xmlText) {
        ...
        for (facette in queryParams.findAll { it.value }) { 
            def qpd3 = nextRepetition(qry.QPD[3])
            qpd3[1].value = facette.key
            qpd3[2].value = facette.value
        }
    ...
    private static String constructQueryId(GPathResult source) {
