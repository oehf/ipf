/**
 * IntracaudalRoute.java
 * <p>
 * File generated from the voc::IntracaudalRoute uml Enumeration
 * Generated by IHE - europe, gazelle team
 */
package net.ihe.gazelle.hl7v3.voc;

import jakarta.xml.bind.annotation.XmlEnum;
import jakarta.xml.bind.annotation.XmlEnumValue;
import jakarta.xml.bind.annotation.XmlRootElement;
import jakarta.xml.bind.annotation.XmlType;
/**
 * Description of the enumeration IntracaudalRoute.
 *
 */

@XmlType(name = "IntracaudalRoute")
@XmlEnum
@XmlRootElement(name = "IntracaudalRoute")
public enum IntracaudalRoute {
	@XmlEnumValue("ICAUDINJ")
	ICAUDINJ("ICAUDINJ");
	
	private final String value;

    IntracaudalRoute(String v) {
        value = v;
    }
    
     public String value() {
        return value;
    }

    public static IntracaudalRoute fromValue(String v) {
        for (IntracaudalRoute c: IntracaudalRoute.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }
	
}