/**
 * UrinaryBladderIrrigation.java
 *
 * File generated from the voc::UrinaryBladderIrrigation uml Enumeration
 * Generated by IHE - europe, gazelle team
 */
package net.ihe.gazelle.hl7v3.voc;

import jakarta.xml.bind.annotation.XmlEnum;
import jakarta.xml.bind.annotation.XmlEnumValue;
import jakarta.xml.bind.annotation.XmlRootElement;
import jakarta.xml.bind.annotation.XmlType;
/**
 * Description of the enumeration UrinaryBladderIrrigation.
 *
 */

@XmlType(name = "UrinaryBladderIrrigation")
@XmlEnum
@XmlRootElement(name = "UrinaryBladderIrrigation")
public enum UrinaryBladderIrrigation {
	@XmlEnumValue("BLADIRR")
	BLADIRR("BLADIRR"),
	@XmlEnumValue("BLADIRRC")
	BLADIRRC("BLADIRRC"),
	@XmlEnumValue("BLADIRRT")
	BLADIRRT("BLADIRRT");
	
	private final String value;

    UrinaryBladderIrrigation(String v) {
        value = v;
    }
    
     public String value() {
        return value;
    }

    public static UrinaryBladderIrrigation fromValue(String v) {
        for (UrinaryBladderIrrigation c: UrinaryBladderIrrigation.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }
	
}