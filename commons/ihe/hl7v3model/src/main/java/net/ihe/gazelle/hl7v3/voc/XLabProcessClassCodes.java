/**
 * XLabProcessClassCodes.java
 * <p>
 * File generated from the voc::XLabProcessClassCodes uml Enumeration
 * Generated by IHE - europe, gazelle team
 */
package net.ihe.gazelle.hl7v3.voc;

import jakarta.xml.bind.annotation.XmlEnum;
import jakarta.xml.bind.annotation.XmlEnumValue;
import jakarta.xml.bind.annotation.XmlRootElement;
import jakarta.xml.bind.annotation.XmlType;
/**
 * Description of the enumeration XLabProcessClassCodes.
 *
 */

@XmlType(name = "XLabProcessClassCodes")
@XmlEnum
@XmlRootElement(name = "XLabProcessClassCodes")
public enum XLabProcessClassCodes {
	@XmlEnumValue("ACSN")
	ACSN("ACSN"),
	@XmlEnumValue("CONTREG")
	CONTREG("CONTREG"),
	@XmlEnumValue("PROC")
	PROC("PROC"),
	@XmlEnumValue("SPCTRT")
	SPCTRT("SPCTRT"),
	@XmlEnumValue("TRNS")
	TRNS("TRNS");
	
	private final String value;

    XLabProcessClassCodes(String v) {
        value = v;
    }
    
     public String value() {
        return value;
    }

    public static XLabProcessClassCodes fromValue(String v) {
        for (XLabProcessClassCodes c: XLabProcessClassCodes.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }
	
}