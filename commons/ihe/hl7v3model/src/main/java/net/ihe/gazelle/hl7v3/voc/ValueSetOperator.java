/**
 * ValueSetOperator.java
 * <p>
 * File generated from the voc::ValueSetOperator uml Enumeration
 * Generated by IHE - europe, gazelle team
 */
package net.ihe.gazelle.hl7v3.voc;

import jakarta.xml.bind.annotation.XmlEnum;
import jakarta.xml.bind.annotation.XmlEnumValue;
import jakarta.xml.bind.annotation.XmlRootElement;
import jakarta.xml.bind.annotation.XmlType;
/**
 * Description of the enumeration ValueSetOperator.
 *
 */

@XmlType(name = "ValueSetOperator")
@XmlEnum
@XmlRootElement(name = "ValueSetOperator")
public enum ValueSetOperator {
	@XmlEnumValue("E")
	E("E"),
	@XmlEnumValue("I")
	I("I");
	
	private final String value;

    ValueSetOperator(String v) {
        value = v;
    }
    
     public String value() {
        return value;
    }

    public static ValueSetOperator fromValue(String v) {
        for (ValueSetOperator c: ValueSetOperator.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }
	
}