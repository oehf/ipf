...
public class RestClientAuditMessageQueueTest {

    private int port;
    private ClientAndServer mockServer;
    private RestClientAuditMessageQueue atnaQueue;
    ...
    public void testSuccessfulAudit() {
        try (var server = new MockServerClient("127.0.0.1", port)) {
            ...
            // Setup producer
            atnaQueue = new RestClientAuditMessageQueue(
                RestClient.builder(),
    ...
    public void testAuditSomewhere() {
        atnaQueue = new RestClientAuditMessageQueue(
            RestClient.builder(),
            URI.create("http://localhost:" + freePort() + "/audit"));
