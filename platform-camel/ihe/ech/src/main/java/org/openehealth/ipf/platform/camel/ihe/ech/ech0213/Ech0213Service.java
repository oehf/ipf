...
@Slf4j
public class Ech0213Service extends AbstractWebService implements Ech0213PortType {

    @Override
    public Response manageSpid(Request request) {
        Exchange exchange = process(request);
        Exception exception = Exchanges.extractException(exchange);
        if (exception != null) {
            log.debug("eCH-0213 request failed", exception);
