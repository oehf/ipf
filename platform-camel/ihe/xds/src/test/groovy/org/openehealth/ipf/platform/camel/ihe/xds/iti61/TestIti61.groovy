...
class TestIti61 extends StandardTestContainer {
    ...
    @Test
    void testIti61() {
        assert Status.SUCCESS == sendIt(SERVICE1, 'service 1').status
        assert Status.SUCCESS == sendIt(SERVICE2, 'service 2').status
    ...
    void testIti61FailureAudit() {
        assert Status.FAILURE == sendIt(SERVICE2, 'falsch').status
