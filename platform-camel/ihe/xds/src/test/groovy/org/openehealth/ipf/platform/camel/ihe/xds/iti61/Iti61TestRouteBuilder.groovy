...
import static org.openehealth.ipf.platform.camel.ihe.xds.XdsCamelValidators.ITI_61_REQUEST_VALIDATOR
import static org.openehealth.ipf.platform.camel.ihe.xds.XdsCamelValidators.ITI_61_RESPONSE_VALIDATOR

public class Iti61TestRouteBuilder extends SpringRouteBuilder {
    @Override
    public void configure() throws Exception {
        from('xds-iti61:xds-iti61-service1')
            .process(ITI_61_REQUEST_VALIDATOR)
            .process { checkValue(it, 'service 1') }
            .process(ITI_61_RESPONSE_VALIDATOR)
    ...
    void checkValue(exchange, expected) {
        ...
        def response = new Response(expected == value ? Status.SUCCESS : Status.FAILURE)
